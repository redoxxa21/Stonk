...
public class PortfolioController {
    ...
    @GetMapping("/{userId}")
    public ResponseEntity<List<HoldingResponse>> getPortfolio(@PathVariable Long userId) {
        return ResponseEntity.ok(portfolioService.getPortfolio(userId));
    }

    @GetMapping("/{userId}/holding/{symbol}")
    public ResponseEntity<HoldingResponse> getHolding(@PathVariable Long userId, @PathVariable String symbol) {
        return ResponseEntity.ok(portfolioService.getHolding(userId, symbol));
    }
